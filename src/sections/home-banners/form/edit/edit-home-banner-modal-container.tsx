...
import EditHomeBannerFormContainer from "./edit-home-banner-form-container";

export default function EditHomeBannerModalContainer() {
  const { getInfoModal } = useContext(ModalContext);
  const infoModal = getInfoModal(modalTypes.editHomeBannerModal.name);
  ...
  return (
    <div className="flex flex-1 flex-col h-full w-full">
      {!loading ? (
        homeBanner && !error ? (
          <EditHomeBannerFormContainer homeBanner={homeBanner} />
        ) : (
