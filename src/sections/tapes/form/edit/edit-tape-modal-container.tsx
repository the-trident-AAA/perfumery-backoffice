...
import EditTapeFormContainer from "./edit-tape-form-container";
import useTape from "../../hooks/use-tape";

export default function EditTapeModalContainer() {
  const { getInfoModal } = useContext(ModalContext);
  const infoModal = getInfoModal(modalTypes.editTapeModal.name);
  ...
  return (
    <div className="flex flex-1 flex-col h-full w-full">
      {!loading ? (
        tape && !error ? (
          <EditTapeFormContainer tape={tape} />
        ) : (
